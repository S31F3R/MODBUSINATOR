...
def regsToFloat(msw: int, lsw: int) -> float | None:
    """Decode two registers (MSW-first, big-endian) → IEEE 754 float (exactly how MODBUSINATOR stores them)"""
    try:
...
def main():
    ...
    parser.add_argument("--mode", choices=["tcp", "serial"], default="tcp",
                        help="tcp or serial (default: tcp)")
    ...
    parser.add_argument("--maxAddr", type=int, default=1024,
                        help="Maximum address to scan (safe default; MODBUSINATOR uses \~612)")
    parser.add_argument("--chunk", type=int, default=100,
                        help="Registers per read request (max \~125 on most devices)")
    parser.add_argument("--decodeFloats", action="store_true",
                        help="Also decode every 2 registers as float (perfect for MODBUSINATOR)")
    ...
    while addr <= args.maxAddr:
        ...
        try:
            result = client.read_holding_registers(address=addr, count=count, slave=args.unit)
    ...
    if args.decodeFloats and lastGood >= 1:
        print("\n=== Decoded Floats (MSW-first, exactly as MODBUSINATOR stores them) ===")
